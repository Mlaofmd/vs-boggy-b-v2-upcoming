...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		while (!FileSystem.exists(directories[0] + "hscripts/states/CreditsState.hx") && directories.length > 0)
			directories.shift();

		if (directories.length > 0)
			hscript = new FunkinHScript(directories[0] + "hscripts/states/CreditsState.hx");
		else
	...
	#if MODS_ALLOWED
	public var modsAdded:Array<String> = [];
	public function pushModCreditsToList(folder:String)
	{
		...
		#if HSCRIPT_ALLOWED
		hscript.call("pushModCreditsToList", [folder]);
		#end
	}
	#end
