...
class StoryMenuState extends MusicBeatState {
	...
	override function create()
	{
		...
		rankText.setFormat(Paths.font("vcr.ttf"), scoreText.size, FlxColor.WHITE, CENTER);
		...
		while (!FileSystem.exists(directories[0] + "hscripts/states/StoryMenuState.hx") && directories.length > 0)
			directories.shift();

		if (directories.length > 0)
			hscript = new FunkinHScript(directories[0] + "hscripts/states/StoryMenuState.hx");
		else
	...
	public function changeDifficulty(change:Int = 0) {
		...
		#if HSCRIPT_ALLOWED
		hscript.call("changeDifficulty", [change]);
		#end
	}
	...
	public function changeWeek(change:Int = 0) {
		...
		#if HSCRIPT_ALLOWED
		hscript.call("changeWeek", [change]);
		#end
	}
	...
	public function updateText() {
		...
		var rank:String = "?";
		for (rating in PlayState.ratingStuff) {
			if (rank == "?" && Highscore.getWeekRating(leWeek.fileName, curDifficulty) >= rating[1])
				rank = rating[0];
		}
