...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		while (!FileSystem.exists(directories[0] + "hscripts/states/FreeplayState.hx") && directories.length > 0)
			directories.shift();

		if (directories.length > 0)
			hscript = new FunkinHScript(directories[0] + "hscripts/states/FreeplayState.hx");
		else
	...
	override function update(elapsed:Float)
	{
		...
		else if (FlxG.keys.anyJustPressed(ClientPrefs.copyKey(ClientPrefs.data.keyBinds["debug_1"]))) {
			persistentUpdate = false;
			var songLowercase:String = Paths.formatToSongPath(songs[curSelected].songName);
			var poop:String = Highscore.formatSong(songLowercase, curDifficulty);
			/*#if MODS_ALLOWED
