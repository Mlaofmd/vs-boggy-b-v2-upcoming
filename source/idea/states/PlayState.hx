...
#end
import idea.system.song.Section.SwagSection;
import idea.system.song.Song.SwagSong;
...
import idea.objects.gameplay.notes.Note.EventNote;
...
import idea.data.StageData;
import idea.scripts.FunkinLua;
import idea.cutscenes.DialogueBoxPsych;
import idea.utils.Conductor.Rating;

/*#if !flash
import idea.effects.LuaShader;
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff(get, never):Array<Array<Dynamic>>;
	public static function get_ratingStuff():Array<Array<Dynamic>> {
		var returnValue:Array<Array<Dynamic>> = [];

		var rawRatings:Array<String> = CoolUtil.coolTextFile(Paths.txt("ratingList"));
		for (rating in rawRatings) {
			var name:String = rating.replace(" | ", "|").split("|")[0];
			var percent:String = rating.replace(" | ", "|").split("|")[1];

			returnValue.push([name, percent]);
		}
	...
	public var luaShaders:Map<String, LuaShader> = [];
	...
	public var grpRegularSplashes:FlxTypedGroup<NoteSplash>;
	public var grpSustainSplashes:FlxTypedGroup<NoteSplash>;
	...
	public static var campaignCombo:Int = 0;
	...
	var precacheList:Map<String, String> = [];
	...
	override public function create()
	{
		...
		timeBar.createFilledBar(emptyColor, fillColor);
		timeBar.numDivisions = FlxG.width;
		timeBar.alpha = 0;
		timeBar.visible = ClientPrefs.data.showTimeBar;
		add(timeBar);
		...
		grpSustainSplashes = new FlxTypedGroup<NoteSplash>();
		...
		grpRegularSplashes = new FlxTypedGroup<NoteSplash>();
		add(grpRegularSplashes);

		var splash:NoteSplash = new NoteSplash(-2000, -2000, 0);
		splash.alpha = 0;
		grpRegularSplashes.add(splash);

		var splash:NoteSplash = new NoteSplash(-2000, -2000, 0, true);
		splash.alpha = 0;
		...
		if (ClientPrefs.data.classicScoreText) {
			scoreTxt = new FlxText(healthBarBG.x + healthBarBG.width - 190, healthBarBG.y + 14, 0, "", 20);
		...
		botplayTxt = new FlxText(400, ClientPrefs.data.downScroll ? 606.25 : 739.25, FlxG.width - 800, "BOTPLAY", 32);
	...
	/*#if (!flash && sys)
	public var runtimeShaders:Map<String, String> = [];
	public function createRuntimeShader(name:String):LuaShader {
		#if SHADERS_ALLOWED
		if (!runtimeShaders.exists(name) && !initLuaShader(name)) {
			FlxG.log.warn('Shader $name not found!');
			return new LuaShader();
		}

		return new LuaShader(runtimeShaders[name]);
		#else
	...
	public function initLuaShader(name:String):Bool {
		...
		for (folder in foldersToCheck) {
			...
			#end
			{
				var file:String = folder + name + ".frag";
				...
				if (found) {
					runtimeShaders.set(name, file);
					return true;
				}
			}
		}
		FlxG.log.warn("Missing shader " + name + "!");
		return false;
	}
	#end*/
	...
	public function reloadHealthBarColors() {
		var leftColor:FlxColor = ClientPrefs.data.healthBarColors ? FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]) : 0xFFFF0000;
		var rightColor:FlxColor = ClientPrefs.data.healthBarColors ? FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]) : 0xFF66FF33;
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 1:
						...
						insert(members.indexOf(regularNotes), countdownReady);
					...
					case 2:
						...
						insert(members.indexOf(regularNotes), countdownSet);
					...
					case 3:
						...
						insert(members.indexOf(regularNotes), countdownGo);
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC, true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
	...
	override function update(elapsed:Float)
	{
		...
		if (healthBar.percent < 20) {
			if (iconP1.animation.exists("lose-loop"))
				iconP1.playAnim("lose-loop");
			else
				iconP1.playAnim("lose", iconP1.animation.getByName("lose").numFrames - 1);

			if (iconP1.animation.exists("win-loop"))
				iconP1.playAnim("win-loop");
			else
				iconP1.playAnim("win", iconP1.animation.getByName("win").numFrames - 1);
			...
			if (iconP1.animation.exists("normal-loop"))
				iconP1.playAnim("normal-loop");
			else
				iconP1.playAnim("normal", iconP1.animation.getByName("normal").numFrames - 1);
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				if (dunceNote.isSustainNote)
					sustainNotes.insert(0, dunceNote);
				else
					regularNotes.insert(0, dunceNote);
		...
		setOnLuas("cameraX", FlxG.camera.viewX);
		setOnLuas('cameraY', FlxG.camera.viewY);
	...
	public function openOptionsMenu() {
		...
		openSubState(new idea.states.menus.options.OptionsMenu(true));
		...
		switch(eventName) {
			...
			case 'Set Property':
				try {
					...
					if (split.length > 1)
						FunkinLua.setVarInArray(FunkinLua.getPropertyLoopThingWhatever(split, true, true), split[split.length-1], value2);
					else
			...
			case "Play Sound":
				...
				volume = Math.isNaN(volume) ? volume : 1;
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode) {
				...
				campaignBads += bads;
				campaignShits += shits;
				if (campaignCombo < highestCombo)
					campaignCombo = highestCombo;
				campaignRatings.push(ratingPercent);

				storyPlaylist.remove(storyPlaylist[0]);

				if (storyPlaylist.length == 0) {
					for (value in campaignRatings)
						campaignRating += value;
					campaignRating /= campaignRatings.length;
	...
	function startAchievement(achieve:String) {
		Main.achieveVar.giveAchievement(achieve, achievementEnd);
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (note.isSustainNote)
				spawnSustainNoteSplash(spr.getMidpoint().x, spr.getMidpoint().y, note.noteData, spr, note);
	...
	function killHenchmen():Void
	{
		if(!ClientPrefs.data.lowQuality && ClientPrefs.data.violence && curStage == 'limo') {
			if(limoKillingState < 1) {
				...
				#if ACHIEVEMENTS_ALLOWED
				AchievementData.henchmenDeath++;
				FlxG.save.data.henchmenDeath = AchievementData.henchmenDeath;
				...
				FlxG.log.add('Deaths: ' + AchievementData.henchmenDeath);
				#end
			}
		}
	}
	...
	override function beatHit()
	{
		...
		if (healthBar.percent < 20) {
			if (iconP1.animation.exists("lose"))
				iconP1.playAnim("lose");

			if (iconP1.animation.exists("win"))
				iconP2.playAnim("win");
		} else if (healthBar.percent > 80) {
			if (iconP1.animation.exists("win"))
				iconP1.playAnim("win");
			...
			if (iconP1.animation.exists("normal"))
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if (!AchievementData.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				if (unlock) {
					AchievementData.unlockAchievement(achievementName);
	...
	public function applySettings() {
		strumLine.y = (ClientPrefs.data.downScroll ? FlxG.height - 150 : 50);
		timeBar.y = ClientPrefs.data.downScroll ? FlxG.height - 8 : 0;
		healthBarBG.y = (ClientPrefs.data.downScroll ? 0.11 * FlxG.height : FlxG.height * 0.89);
		healthBar.y = healthBarBG.y + 4;
		iconP1.y = iconP2.y = healthBar.y - 75;
		botplayTxt.y = ClientPrefs.data.downScroll ? 606.25 : 739.25;
		for (strum in strumLineNotes.members) {
			strum.y = strumLine.y;
			strum.downScroll = ClientPrefs.data.downScroll;
		}
		for (unspawnNote in unspawnNotes) {
			if (unspawnNote.isSustainNote)
				unspawnNote.flipY = ClientPrefs.data.downScroll;
		}
		for (note in notes.members) {
			if (note.isSustainNote)
				note.flipY = ClientPrefs.data.downScroll;
		}
		for (note in notes) {
			note.distance = ((0.45 * (ClientPrefs.data.downScroll ? 1 : -1)) * (Conductor.songPosition - note.strumTime) * songSpeed * note.multSpeed);
			...
			if(note.copyX)
				note.x = strum.x + Math.cos(angleDir) * note.distance;

			if(note.copyY)
			{
				note.y = strum.y + Math.sin(angleDir) * note.distance;

				//Jesus fuck this took me so much mother fucking time AAAAAAAAAA
				if(ClientPrefs.data.downScroll && note.isSustainNote)
				{
					if (note.animation.curAnim.name.endsWith('end')) {
						note.y += 10.5 * (Conductor.crochet / 400) * 1.5 * songSpeed + (46 * (songSpeed - 1));
						note.y -= 46 * (1 - (Conductor.crochet / 600)) * songSpeed;
						if (PlayState.isPixelStage)
							note.y += 8 + (6 - note.originalHeightForCalcs) * PlayState.daPixelZoom;
						else
							note.y -= 19;
					}
					note.y += (Note.swagWidth / 2) - (60.5 * (songSpeed - 1));
	...
	public function set_combo(value:Int) {
		combo = value;
	...
	public function closeResultsMenu() {
		if (isStoryMode) {
