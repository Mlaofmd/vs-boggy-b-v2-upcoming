...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuState.ideaEngineVersion.trim());
		...
		set("showTimeBar", ClientPrefs.data.showTimeBar);
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				var color:Int = CoolUtil.colorFromString(targetColor);
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int, height:Int, color:String) {
			var colorNum:Int = CoolUtil.colorFromString(color);
		...
		Lua_helper.add_callback(lua, "setCameraShaders", function(cam:String, names:Array<String>) {
			#if (!flash && SHADERS_ALLOWED && sys)
			var camera:FlxCamera = cameraFromString(cam);
			var shaders:Array<BitmapFilter> = [];

			for (i in 0...names.length) {
				if (!PlayState.instance.runtimeShaders.exists(names[i]) && !initLuaShader(names[0])) {
					luaTrace("setCameraShaders: Shader " + names[0] + " is missing!", false, false, FlxColor.RED);
				}
				var arr:Array<String> = PlayState.instance.runtimeShaders.get(names[0]);
				shaders.push(new ShaderFilter(new FlxRuntimeShader(arr[0], arr[1])));
			}
			camera.setFilters(shaders);
			return true;
			#else
			luaTrace("setCameraShaders: Platform unsupported for Runtime Shaders!", false, false, FlxColor.RED);
			#end
			return false;
		});

		Lua_helper.add_callback(lua, "addShaderToCamera", function(name:String, cam:String) {
			#if (!flash && SHADERS_ALLOWED && sys)
			@:privateAccess {
				var camera:FlxCamera = cameraFromString(cam);
				...
				if (!PlayState.instance.runtimeShaders.exists(name) && !initLuaShader(name)) {
					luaTrace("addShaderToCamera: Shader " + name + " is missing!", false, false, FlxColor.RED);
					return false;
				}
			...
			#else
			luaTrace("addShaderToCamera: Platform unsupported for Runtime Shaders!", false, false, FlxColor.RED);
			#end
			return false;
		});
