...
class Highscore
{
	...
	public static var weekRating:Map<String, Float> = [];
	...
	public static function saveWeekRating(week:String, rating:Float, ?diff:Int = 0) {
		var daWeek:String = formatSong(week, diff);

		if (weekRating.exists(daWeek)) {
			if (weekRating[daWeek] < rating)
				setWeekRating(daWeek, rating);
		} else
			setWeekRating(daWeek, rating);
	}

	public static function setWeekRating(week:String, rating:Float) {
