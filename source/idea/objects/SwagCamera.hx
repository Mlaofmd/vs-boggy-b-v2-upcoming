...
class SwagCamera extends FlxCamera {
    ...
    public function new(x:Int = 0, y:Int = 0, width:Int = 0, height:Int = 0, zoom:Float = 0) {
		...
		FlxG.signals.gameResized.add(resetCache);
    }
	...
	override function update(elapsed:Float) {
		...
		if (flashSprite.width != width || flashSprite.height != height)
			resetCache();
	}
	...
	override function destroy() {
		FlxG.signals.gameResized.remove(resetCache);
		super.destroy();
	}

	private function resetCache(?width:Int, ?height:Int) {
		@:privateAccess {
